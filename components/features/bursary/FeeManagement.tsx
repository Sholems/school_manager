                        ...
                        {filteredStudents.map(s => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`p-3 rounded-md cursor-pointer flex justify-between items-center text-sm transition-colors ${selectedStudent === s.id
                                            ? 'bg-brand-50 border-brand-200 border'
                                            : 'hover:bg-gray-50'
                                        }`}
                                >
                                    <div>
                                        <div className="font-medium">{s.names}</div>
                                        <div className="text-xs text-gray-500">{s.student_no}</div>
                                    </div>
                                    <div className={`font-mono text-sm ${bal > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                        {bal > 0 ? `-${Utils.formatCurrency(bal)}` : '✓ Paid'}
                                    </div>
                                </div>
                            );
                        })}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            <Card className="bg-blue-50 border-blue-100 p-4">
                                <div className="text-sm text-blue-600">Total Bill</div>
                                <div className="text-xl font-bold text-blue-900">{Utils.formatCurrency(totalBill)}</div>
                            </Card>
                            <Card className="bg-green-50 border-green-100 p-4">
                                <div className="text-sm text-green-600">Total Paid</div>
                                <div className="text-xl font-bold text-green-900">{Utils.formatCurrency(totalPaid)}</div>
                            </Card>
                            <Card className={`p-4 ${balance > 0 ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'}`}>
                                <div className={`text-sm ${balance > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                    {balance > 0 ? 'Outstanding' : 'Status'}
                                </div>
                                <div className={`text-xl font-bold ${balance > 0 ? 'text-red-900' : 'text-green-900'}`}>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {applicableFees.map(fee => (
                                        <tr key={fee.id}>
                                            ...
                                            <td className="px-4 py-2 text-right font-mono">{Utils.formatCurrency(fee.amount)}</td>
                                        </tr>
                                    ))}
                                    <tr className="bg-gray-50 font-bold">
                                        <td className="px-4 py-2">Total Billed</td>
                                        <td className="px-4 py-2 text-right">{Utils.formatCurrency(totalBill)}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </Card>
